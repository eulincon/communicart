...
const UserProfile = () => {
  ...
  async function handleSelecionarCandidato(candidaturaId) {
    await api.patch(`api/vagas/selecionar_candidato/${candidaturaId}`)
    .then(response => {
  ...
  useEffect(() => {
    ...
    api.defaults.headers.Authorization = `Bearer ${localStorage.getItem('@RNAuth:token')}`
  ...
  function socialMediaLink(midia) {
    ...
    if (site === "facebook") {
      return (
        <a href={`${link}`} target="_blank" rel="noreferrer" className="social-media-link">
      ...
      <SkeletonPage footer={true} sidebar={true}>
        ...
        <div className="text-light bg-secondary p-3 rounded">
          <h3 className="text-center">Proposta</h3>
            ...
            <p>Data de entrega: {proposta.deliveryDate}</p>
            <p>Observações: {proposta.observations}</p>
            <div className="d-flex justify-content-center">
              <button onClick={() => handleSelecionarCandidato(proposta.id)} className="btn btn-secondary_">Selecionar candidato</button>
            </div>
        </div>
      </SkeletonPage>
    );
